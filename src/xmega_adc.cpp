...
{
    adc->REFCTRL=ref_vcc;
    //ADCA.REFCTRL = ADC_REFSEL_INT1V_gc; // Внутреннее опорное напряжение 1.00В
    // ADCA.REFCTRL = ADC_REFSEL_VCC_gc;   // Внутреннее напряжение VCC/1.6
    // ADCA.REFCTRL = ADC_REFSEL_AREFA_gc; // Внешнее опорное напряжение с вывода AREF порта А
    // ADCA.REFCTRL = ADC_REFSEL_AREFB_gc; // Внешнее опорное напряжение с вывода AREF порта B
}

// запуск одиночного преобразования в нужном канале
void XmegaAdc::xmega_adc_start_chanel(ADC_CH_t *ch)
{
...
void XmegaAdc::xmega_adc_auto_mode(ADC_t *adc )
{
